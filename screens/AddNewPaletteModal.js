...
export const AddNewPaletteModal = ({ navigation }) => {
  ...
  const handleColorSelect = (data) => {
    const condition = colors.find(
      ({ colorName }) =>
        colorName.toLowerCase() === data.colorName.toLowerCase()
    );

    if (!!condition) {
      const newColors = colors.map((color) => {
        if (color.colorName === data.colorName) {
          return data;
        }
        return color;
      });
  ...
  const handleSubmit = () => {
    const result = colors.filter((color) => color.isEnabled);
    if (!paletteName) {
      Alert.alert('Error', 'Please provide a palette name');
    } else if (result.length < 3) {
      Alert.alert('Error', 'Please select at least 3 colors');
    } else if (!paletteName && !result.length) {
      Alert.alert('Error', 'No palette name or colors selected');
    } else {
      navigation.navigate('Home', { colors: result, paletteName });
    }
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.formControl}>
        <Text style={styles.formControlText}>Name of your color palette</Text>
        <TextInput
      ...
      <FlatList
        ...
        renderItem={(data) => (
          <ColorSwitch {...data.item} onSwitch={handleColorSelect} />
        )}
      />
